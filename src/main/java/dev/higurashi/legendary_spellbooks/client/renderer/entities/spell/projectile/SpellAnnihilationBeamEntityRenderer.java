...
public class SpellAnnihilationBeamEntityRenderer extends EntityRenderer<SpellAnnihilationBeamEntity> {
    ...
    private void renderFlatQuad(int frame, PoseStack poseStack, VertexConsumer consumer, int packedLight) {
        ...
        this.drawVertex(matrix4f, lastEntry, consumer, -1.3f, -1.3f, 0.0f, minU, minV, packedLight);
        this.drawVertex(matrix4f, lastEntry, consumer, -1.3f, 1.3f, 0.0f, minU, maxV, packedLight);
        this.drawVertex(matrix4f, lastEntry, consumer, 1.3f, 1.3f, 0.0f, maxU, maxV, packedLight);
        this.drawVertex(matrix4f, lastEntry, consumer, 1.3f, -1.3f, 0.0f, maxU, minV, packedLight);
    }
    ...
    private void drawBeam(float length, int frame, PoseStack poseStack, VertexConsumer consumer, int packedLight) {
        ...
        this.drawVertex(matrix4f, lastEntry, consumer, -1.0f, 0.0f, 0.0f, minU, minV, packedLight);
        this.drawVertex(matrix4f, lastEntry, consumer, -1.0f, length, 0.0f, minU, maxV, packedLight);
        this.drawVertex(matrix4f, lastEntry, consumer, 1.0f, length, 0.0f, maxU, maxV, packedLight);
        this.drawVertex(matrix4f, lastEntry, consumer, 1.0f, 0.0f, 0.0f, maxU, minV, packedLight);
    }

    private void drawVertex(Matrix4f matrix4f, PoseStack.Pose lastEntry, VertexConsumer consumer, float offsetX, float offsetY, float offsetZ, float textureX, float textureY, int packedLight) {
        consumer.addVertex(matrix4f, offsetX, offsetY, offsetZ)
                .setUv(textureX, textureY)
                .setUv2(LightTexture.block(packedLight), LightTexture.sky(packedLight))
                .setOverlay(OverlayTexture.NO_OVERLAY)
                .setNormal(lastEntry, 0.0f, 1.0f, 0.0f)
                .setColor(0xFFFFFFFF);
    }
}
