...
public class PossessedFallingSoulBladeSpell extends BaseDoubleSchoolSpell {
    private static final ResourceLocation spellResource = ResourceLocation.fromNamespaceAndPath(LegendarySpellbooks.MOD_ID, "possessed_falling_soul_blade");
    ...
    public BaseDoubleSchoolSpell newDoubleSchoolSpell(int newLevel) {
        return new PossessedFallingSoulBladeSpell(newLevel);
    }
    ...
    public void onCast(Level level, int spellLevel, LivingEntity caster, CastSource source, MagicData magicData) {
        ...
        for (int i = 0; i < ringCount; i++) {
            ...
            for (Vec3 spawnPoint : spawnPoints) {
                ...
                FallingSoulBladeEntity blade = new FallingSoulBladeEntity(level, spawnPos.x, spawnPos.y, spawnPos.z, yaw, i * 2, caster, damage, isAnother(spellLevel));
